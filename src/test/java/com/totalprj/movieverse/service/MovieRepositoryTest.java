...
//        movies.add(movie8);
    ...
    @Test
    @DisplayName("박스오피스 영화 정보 저장 테스트")
    public void saveBoxOfficeTest() {
        List<Movie> movie = createTestMovies();
//        List<Movie> testMovie = movieRepository.saveAll(movies);
        Iterable<Movie> testMovies = movieRepository.saveAll(movie);
        em.flush();
        em.clear();
        for (Movie movies : testMovies) {
            System.out.println("박스오피스 영화 정보 : " + movies);
        }
